...
def collect_metrics(config):
    ...
    # Extract log path directly from config
    ...
    with open(log_path, "r") as f:
        for line in f:
            if "Total core hours:" in line:
                core_matrix = float(line.split(":")[-1].strip())
            elif "Max memory usage:" in line:
                mem_matrix = float(line.split(":")[-1].strip().replace("GB", ""))
            elif "PASS -- COMPILE" in line or "PASS -- TEST" in line:
...
def write_csv_and_plot(results):
    # Write summary CSV
    with open("results/summary.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["App", "Compiler", "Hash", "Core Hours", "Memory GB"])
        for result in results:
            writer.writerow([
                result["app"],
                result["compiler"],
                result["hash"],
                result["metrics"]["core_matrix"],
                result["metrics"]["mem_matrix"]
            ])
    ...
    for result in results:
        ...
        compiler_groups.setdefault(compiler, []).append(result)

    # Plot per compiler
    for compiler, group in compiler_groups.items():
        apps = [r["app"] for r in group]
        cores = [r["metrics"]["core_matrix"] for r in group]
        mems = [r["metrics"]["mem_matrix"] for r in group]

        plt.figure(figsize=(10, 5))
        plt.bar(apps, cores, label="Core Hours")
        plt.bar(apps, mems, bottom=cores, label="Memory GB")
        plt.title(f"Resource Usage by App ({compiler})")
        plt.xticks(rotation=45, ha="right")
        plt.legend()
        plt.tight_layout()

        output_dir = f"results/by_compiler"
        os.makedirs(output_dir, exist_ok=True)
        plt.savefig(f"{output_dir}/{compiler}_usage.png")
        plt.close()
...
def process_app_yaml(yaml_file, hashes):
    ...
    app_name = list(config.keys())[0]
    compiler_yaml = config[app_name].get("build", {}).get("compiler", "unknown")

    core_matrix, mem_matrix, compiler_from_log = collect_metrics(config)
    compiler_final = compiler_from_log or compiler_yaml
    ...
    result = {
        ...
        "hash": hashes.get(app_name, "unknown")
    }

    if compiler_from_log and compiler_from_log != compiler_yaml:
        print(f"⚠️ Compiler mismatch for {app_name}: YAML={compiler_yaml}, Log={compiler_from_log}")

    output_dir = "results/by_app"
    os.makedirs(output_dir, exist_ok=True)
    ...
    return compiler_final

def main():
    import glob

    # Discover all test YAMLs
    yaml_files = glob.glob("tests-yamls/configs/by_app/*.yaml")
    hashes = load_hashes()  # Your existing hash loader
    ...
    for yaml_file in yaml_files:
        try:
            compiler = process_app_yaml(yaml_file, hashes)
            result_path = f"results/by_app/{os.path.basename(yaml_file)}"
    ...
    # Write summary CSV and compiler-aware plots
    write_csv_and_plot(results)

    # Optional: print compiler summary
    print("\n📊 Compiler Summary:")
    for compiler, count in compiler_counts.items():
        print(f"  {compiler}: {count} apps")
